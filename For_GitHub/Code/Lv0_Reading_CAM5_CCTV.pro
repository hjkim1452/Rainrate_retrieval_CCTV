      ...
      IF(ct_60sec ge 1)THEN BEGIN
        ...
        FOR f = 0, ct_60sec-1L DO BEGIN
          ...
          IF(n_label gt 0.0)THEN BEGIN
            ...
            For l = 0, n_label-1L do begin  ;;; Label로 분류된 입자 하나씩 처리해보기

              label_number = label_arr[l+1] ;;; 배경은 고려하지 않음('+1' & 0값은 배경)
              ...
              Dia_short[l] = abs(max(ydim_label_edge_ang)) + abs(min(ydim_label_edge_ang))  ;;; 단축
              Dia_long[l] = abs(max(xdim_label_edge_ang)) + abs(min(xdim_label_edge_ang))   ;;; 장축
